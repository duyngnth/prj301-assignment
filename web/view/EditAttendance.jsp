<%-- 
    ...
    Created on : 17-Jul-2022, 15:54:57
    Author     : duyng
--%>
                    ...
                    <table class="table table-bordered">
                        ...
                        <c:forEach items="${session.group.students}" var="st" varStatus="st_loop">
                            <tr>
                                ...
                                <td class="text-left">
                                    <input required type="radio" name="status${st.id}" value="Attended"
                                           <c:if test="${atds.get(st_loop.index).status eq 'Attended'}">
                                               checked="checked"
                                           </c:if>
                                           >Present<br>
                                    <input required type="radio" name="status${st.id}" value="Absent"
                                           <c:if test="${atds.get(st_loop.index).status eq 'Absent'}">
                                               checked="checked"
                                           </c:if>
                                           >Absent
                                </td>
                                <td class="text-center">
                                    <input type="text" name="comment${st.id}" value="${atds.get(st_loop.index).description}">
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
